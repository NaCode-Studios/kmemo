...
- Nine guards against false cache hits: `NumericGuard`, `UnitGuard`, `TemporalGuard`,
  `NegationGuard`, `AntonymGuard`, `EntityGuard`, `ScopeGuard`, `DirectionGuard`,
  `LexicalDivergenceGuard`, plus the `MatchGuards.standard()` / `strict()` / `none()` presets.
